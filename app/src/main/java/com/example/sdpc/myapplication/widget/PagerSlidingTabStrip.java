...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	/*
	 * -------------------------------一下代码主要是用于解决在滑动过程中两边信息显示隐藏和显示问题--------------
	 * -------------------------------------
	 */
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		// arrowShowOrHidden();
		if (!mActivity.isFinishing() && mLeftView != null && mRightView != null) {
			if (mPrentView.getWidth() <= mScreenWidth) {
				mLeftView.setVisibility(View.INVISIBLE);
				// mLeftView.startAnimation(mAnimationOut);
		...
		if (l + mPrentView.getWidth() >= computeHorizontalScrollRange()) {
			mLeftView.setVisibility(View.VISIBLE);
			// mLeftView.startAnimation(mAnimationIn);
			mRightView.setVisibility(View.INVISIBLE);
			return;
		}
		mLeftView.setVisibility(View.VISIBLE);
	...
	public void setParams(Activity activity, View parentView, int screenWidth,
			View leftView, View rightView
	// , Animation animationOut, Animation animationIn
	) {
		mActivity = activity;
		mPrentView = parentView;
	 ...
	 * 判断左右阴影的显示隐藏效果
	 * */
	public void arrowShowOrHidden() {
		// if (!mActivity.isFinishing() && mParentView != null) {
		if (!mActivity.isFinishing()) {
			measure(0, 0);
			// 如果整体宽度小于屏幕宽度的话，那左右阴影都隐藏
			if (mScreenWidth >= getMeasuredWidth()) { // 720 >= 1323
				mLeftView.setVisibility(View.INVISIBLE);
				// mLeftView.startAnimation(mAnimationOut);
				mRightView.setVisibility(View.INVISIBLE);
			}
