...
public class CountingLruMap<K, V> {
  ...
  @GuardedBy("this")
    private final LinkedHashMap<K, V> mMap = new LinkedHashMap<K, V>();
  @GuardedBy("this")
  ...
  synchronized ArrayList<K> getKeys() {
        return new ArrayList<K>(mMap.keySet());
  }

  @VisibleForTesting
  synchronized ArrayList<V> getValues() {
        return new ArrayList<V>(mMap.values());
  }
  ...
  /** Gets the all matching elements. */
    public synchronized ArrayList<LinkedHashMap.Entry<K, V>> getMatchingEntries(@Nullable Predicate<K> predicate) {
        ArrayList<LinkedHashMap.Entry<K, V>> matchingEntries = new ArrayList<Entry<K, V>>();
  ...
  public synchronized ArrayList<V> clear() {
        ArrayList<V> oldValues = new ArrayList<V>(mMap.values());
    mMap.clear();
    mSizeInBytes = 0;
    return oldValues;
  }
