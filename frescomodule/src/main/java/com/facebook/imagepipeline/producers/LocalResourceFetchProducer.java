...
public class LocalResourceFetchProducer extends LocalFetchProducer {
  ...
  protected EncodedImage getEncodedImage(ImageRequest imageRequest) throws IOException {
    return getEncodedImage(
            getResources(imageRequest).openRawResource(getResourceId(imageRequest)),
            getLength(imageRequest));
  }

  private Resources getResources(ImageRequest imageRequest) {
    Context context = imageRequest.getWidgetContex();
    return context == null ? mResources : context.getResources();
  }

  private int getLength(ImageRequest imageRequest) {
    AssetFileDescriptor fd = null;
    try {
      fd = getResources(imageRequest).openRawResourceFd(getResourceId(imageRequest));
