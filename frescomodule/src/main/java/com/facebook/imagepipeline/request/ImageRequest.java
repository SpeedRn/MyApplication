...
public class ImageRequest {
  ...
  private final Context mWidgetContex;
  ...
  public static ImageRequest fromUri(@Nullable Uri uri, Context context) {
    return (uri == null) ? null :
            ImageRequestBuilder.newBuilderWithSource(uri).setWidgetContext(context).build();
  }

  public static ImageRequest fromUri(@Nullable String uriString, Context context) {
    return (uriString == null || uriString.length() == 0) ?
            null : fromUri(Uri.parse(uriString), context);
  }

  protected ImageRequest(ImageRequestBuilder builder) {
    mImageType = builder.getImageType();
    mSourceUri = builder.getSourceUri();
    mWidgetContex = builder.getWidgetContex();
  ...
  public Context getWidgetContex() {
    return mWidgetContex;
  }
